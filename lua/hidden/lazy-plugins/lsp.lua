    ...
    config = function()
            ...
            handlers = {
                ...
                ["omnisharp"] = function()
                    ...
                    local on_attach = function(client, bufnr)
                        ...
                        vim.keymap.set('n', '<space>vrn', '<cmd>lua vim.lsp.buf.rename()<CR>',
                            { buffer = bufnr, remap = false, desc = "rename omnisharp (lsp)" })
                        vim.keymap.set('n', '<space>vca', '<cmd>lua vim.lsp.buf.code_action()<CR>',
                            { buffer = bufnr, remap = false, desc = "code action omnisharp (lsp)" })
                        vim.keymap.set('n', '<space>vrr', '<cmd>lua vim.lsp.buf.references()<CR>',
                            { buffer = bufnr, remap = false, desc = "references omnisharp (lsp)" })
                        vim.keymap.set('n', '<space>vf', '<cmd>lua vim.lsp.buf.formatting()<CR>',
        ...
        lsp_zero.on_attach(function(client, bufnr)
            ...
            vim.keymap.set("n", "gd", function() vim.lsp.buf.definition() end,
                { buffer = bufnr, remap = false, desc = "jump definition (lsp)" })
            vim.keymap.set("n", "<leader>vws", function() vim.lsp.buf.workspace_symbol() end,
                { buffer = bufnr, remap = false, desc = "work space (lsp)" })
            vim.keymap.set("n", "<leader>vd", function() vim.diagnostic.open_float() end,
                { buffer = bufnr, remap = false, desc = "open diagnostic (lsp)" })
            vim.keymap.set({ "n", "x" }, "<leader>vf", function() vim.lsp.buf.format() end,
                { buffer = bufnr, remap = false, desc = "format code (lsp)" })
